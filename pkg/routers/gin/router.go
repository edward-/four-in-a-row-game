...
func NewRouter(doc *openapi3.T) (routers.Router, error) {
	...
	for _, path := range doc.Paths.InMatchingOrder() {
		...
		for _, s := range servers {
			ginRouter.Match(methods, path, nil)
			// don't validate schemas
			// don't validate host

			r.gins = append(r.gins, routeGin{
				ginRoute:    ginRouter,
...
func (r *Router) FindRoute(req *http.Request) (*routers.Route, map[string]string, error) {
	for i, m := range r.gins {
		routesInfo := m.ginRoute.Routes()

		if matchRoute(routesInfo, req) {
			vars := getQueryParams(req.URL.Query())
			route := *r.routes[i]
...
func getQueryParams(values url.Values) map[string]string {
...
func matchRoute(routesInfo []gin.RouteInfo, req *http.Request) bool {
	for _, r := range routesInfo {
		if r.Path == req.RequestURI && r.Method == req.Method {
			return true
		}
	}
	return false
